  ...
  return (
    <section className="sub-view active">
      ...
      <div className="card">
        ...
        <button className="btn btn-ghost w-full" onClick={() => saveConfig({ social: [...(config.social || []), { uid: uid(), icon: 'globe', handle: '', visible: true }] })} type="button">+ Añadir Red Social</button>
      </div>
      ...
      {/* Minecraft Day Integration */}
      <div className="card">
        <div className="flex items-center justify-between mb-sm">
          <h2 style={{ marginBottom: 0 }}>⛏️ Integración con Minecraft (Días)</h2>
          <button className={`toggle ${config.minecraftEnabled ? 'on' : ''}`} onClick={() => saveConfig({ minecraftEnabled: !config.minecraftEnabled })}></button>
        </div>
        <p className="text-xs text-secondary mb-sm">
          Lee el archivo <code>level.dat</code> de tu mundo local en tiempo real para contar los días transcurridos automáticamente.
        </p>
        <div className="flex-col gap-xs mb-sm">
          <span className="text-xs text-secondary">Nombre de la Carpeta del Mundo:</span>
          <div className="flex gap-sm">
            <input 
              className="inp flex-1" 
              type="text" 
              placeholder="Nombre exacto de la carpeta del mundo..." 
              value={config.minecraftWorldName || ''} 
              onChange={e => saveConfig({ minecraftWorldName: e.target.value })} 
            />
            <button className="btn btn-ghost" onClick={() => showSaved()}>Guardar</button>
          </div>
        </div>
        <div className="flex-col gap-xs">
          <span className="text-xs text-secondary">Ruta opcional de carpeta 'saves' (Vacío para usar la de defecto):</span>
          <input 
            className="inp" 
            type="text" 
            placeholder="Ej: C:\Users\Nombre\AppData\Roaming\.minecraft\saves" 
            value={config.minecraftSavePath || ''} 
            onChange={e => saveConfig({ minecraftSavePath: e.target.value })} 
          />
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="flex gap-xs items-center mb-sm">
          ...
          <button className="btn btn-primary" type="button" style={{ padding: '8px 12px' }} onClick={() => {
      ...
      <div className="card">
        ...
        <div className="flex gap-sm mb-sm" style={{ flexWrap: 'wrap' }}>
          <button className="btn btn-primary" type="button" style={{ flex: 1, padding: '6px 12px' }} onClick={() => window.api.previewAlert({ type: 'follow', user: 'TestUser123', message: '¡@TestUser123 te siguió!' })}>🎉 Follow</button>
          <button className="btn btn-primary" type="button" style={{ flex: 1, padding: '6px 12px' }} onClick={() => window.api.previewAlert({ type: 'gift', user: 'TestUser123', gift: 'Rosa', count: 5, message: '🎁 x5 Rosa' })}>🎁 Regalo</button>
          <button className="btn btn-primary" type="button" style={{ flex: 1, padding: '6px 12px' }} onClick={() => window.api.previewAlert({ type: 'game', message: 'Valorant' })}>🎮 Juego</button>
          <button className="btn btn-primary" type="button" style={{ flex: 1, padding: '6px 12px' }} onClick={() => window.api.previewAlert({ type: 'goal', message: '¡100 seguidores!' })}>🏆 Meta</button>
        </div>
        ...
        <div className="flex-col gap-xs mb-sm mt-md" style={{ background: 'var(--bg-panel)', border: '1px solid var(--border-light)', padding: 12, borderRadius: 8 }}>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-xs items-center">
              ...
              <button className="btn btn-primary" type="button" style={{ padding: '6px 12px' }} onClick={testVoiceLocal}>
                Probar Voz
              </button>
            </div>
          </div>
      ...
      {/* Bot Commands Test Section */}
      <div className="card">
        <h2>🤖 Probar Comandos del Bot (Puntos Ilimitados)</h2>
        <p className="text-xs text-secondary mb-md">
          Simula el envío de un comando de chat para probar la economía y video reacciones. Tendrás 100,000 puntos automáticos para comprar o pedir videos.
        </p>
        
        <div className="flex gap-sm mb-sm" style={{ alignItems: 'center' }}>
          <div className="flex-col gap-xs" style={{ width: 120 }}>
            <span className="text-xs text-secondary">Usuario:</span>
            <input 
              type="text" 
              id="bot-test-user"
              className="inp" 
          ...
          <div className="flex-col gap-xs flex-1">
            <span className="text-xs text-secondary">Comando:</span>
            <input 
              type="text" 
              id="bot-test-command"
              className="inp" 
              placeholder="Ej: !tienda o !comprar gorra o !videor <url>" 
              style={{ height: 32, fontSize: 12 }}
              onKeyDown={async (e) => {
                if (e.key === 'Enter') {
                  const btn = document.getElementById('run-bot-command-btn');
                  if (btn) btn.click();
                }
              }}
            />
        ...
        <button 
          id="run-bot-command-btn"
          className="btn btn-primary w-full" 
          type="button"
          onClick={async () => {
            const cmdInput = document.getElementById('bot-test-command');
            const userInput = document.getElementById('bot-test-user');
            const cmdText = cmdInput?.value?.trim() || '';
            const username = userInput?.value?.trim() || 'Tester';
            if (!cmdText) return;
            
            if (window.api && window.api.testBotCommand) {
              await window.api.testBotCommand(username, cmdText);
              if (cmdInput) cmdInput.value = ''; // Limpiar input sin causar re-render o pérdida de foco
              alert(`Comando "${cmdText}" enviado como @${username}. ¡Verifica el overlay o la cola de reacciones!`);
            } else {
              alert('API de comandos no disponible.');
            }
          }}
        >
          Ejecutar Comando
